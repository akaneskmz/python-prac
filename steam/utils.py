...
TWEET_ACHIEVE_COMPLETE_TAG = os.environ.get("TWEET_ACHIEVE_COMPLETE_TAG")
...
def achieve_unlock_tweet(last_unlock_time):
    ...
    if new_achieve:
        ...
        # 実績コンプタグ
        comp_tag = f"\n#{TWEET_ACHIEVE_COMPLETE_TAG}" if TWEET_ACHIEVE_COMPLETE_TAG else ""
        ...
        for game in games:
            ...
            comp_flg = achieve_num_current == achieve_num_total
            # 進捗テキスト
            ...
            print(media_id)
            tweet_params = {
                'status': f'{game.name}の実績を解除しました。{progress}{tag}{comp_tag if comp_flg else ""}',
                'media_ids': media_id}
